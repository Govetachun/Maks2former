...
class SetCriterion(nn.Module):
    ...
    def _get_binary_mask(self, target):
        y, x = target.size()
        # Ensure that target_onehot has enough channels for all classes in target
        num_classes = max(target.max().item() + 1, self.num_classes + 1)
        target_onehot = torch.zeros(num_classes, y, x)
